...
所有变量在声明时都使用数据类型来限制存储的数据类型。因此，我们可以说数据类型用于告诉变量它们可以存储的数据类型。在 C++ 中定义变量时，编译器根据变量声明的数据类型为该变量分配一些内存。每种数据类型需要不同量的内存。

C++ 支持各种各样的数据类型，程序员可以根据应用程序的需要选择适当的数据类型。数据类型指定要存储的值的大小和类型。然而，每种数据类型的存储表示和机器指令在不同的机器上可能不同，尽管 C++ 指令在所有机器上都是相同的。
...
4. 整数类型 `int`
5. 浮点类型`float`, `double`
6. 空指针类型 `std::nullptr_t` <Tag>C++11</Tag>
...
:::success 补充
[数据类型转换](/docs/lang/cpp/表达式和声明/基本操作)放到表达式的基本操作当中先详细说明.
:::
...
```
这里`&`符号表示[引用](/docs/lang/cpp/变量和数据类型/指针和引用#引用)
