...
$齐次线性方程的通解y=\left\{\begin{array}{llllllllll}C_1e_{}^{\lambda_1x}+C_2e_{}^{\lambda_2x}. & \Delta>0\\ \left(C_1+C_2x\right)e^{\lambda x}. & \Delta=0\\ e^{\alpha x}\left(C_1\cos\beta x+C_2\sin\beta x\right). & \Delta<0\end{array}\right.$
